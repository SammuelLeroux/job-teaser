...
function Home () {
  ...
  // donnees des annonces -> annonces : Id_Annoncess - titre - description - salaire - temps_travail - typecontrat
  ...
  useEffect(() => {
    ...
    if (params.length !== 0)
    {
      annonce = decodeURIComponent(params.get('annonce'));
      entreprise = decodeURIComponent(params.get('entreprise'));
      lieu = decodeURIComponent(params.get('lieu'));
    }
    ...
    const postData = Object.keys(researchAds)
    .map((key) => `${encodeURIComponent(key)}=${encodeURIComponent(researchAds[key])}`)
    .join('&');
    ...
    const getAds = async (urlRequested, postData) => {
      try
      {
